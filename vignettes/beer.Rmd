...
By default, `beer` runs each sample sequentially. However, parallelization is also supported via [`future`](https://cran.r-project.org/web/packages/future/index.html) and [`future.apply`](https://cran.r-project.org/web/packages/future.apply/index.html) packages. Parallelization parameters are specified to both `brew()` and `runEdgeR()` via a character vector or a list of parameters passed to `future::plan()`. For more information on the available evaluation options, we highly recommend this [`future` vignette](https://cran.r-project.org/web/packages/future/vignettes/future-1-overview.html). 

```{r}
## Run edgeR in parallel
runEdgeR(sim_data, parallel = "multisession")
runEdgeR(sim_data, parallel = list(strategy = "multisession", workers = 2))
